...
# static server rule reference for llm context
SERVER_RULES = """
General Rules
(1/3)
...
# log a deletion event and optionally ask llm for a warn suggestion
async def log_deleted_message(bot, message, server_rules, deleter=None):
    ...
    deleter_text = f"{deleter} (`{deleter.id}`)" if deleter else "unknown or self"
...
# resolve log channel from env id
...
# try to read audit logs to see who deleted the message
...
# send deleted content to local llm for a warn call
async def _review_with_llm(message, server_rules):
    ...
    try:
        ...
        return {"recommend": "no", "explanation": "llm unavailable"}

    try:
        content = data["choices"][0]["message"]["content"].strip()
    except (KeyError, IndexError, AttributeError):
        return {"recommend": "no", "explanation": "llm response malformed"}
